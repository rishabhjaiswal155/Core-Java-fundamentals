...
public interface ProductUtils {
	...
	static Stream<Product> filterAndSortedProduct(Map<Integer,Product> maps,String category){
		Category cat=Category.valueOf(category.toUpperCase());
		return maps.values()//Collection<Product>
				   .stream()//Stream<Product>
				   .filter(p->p.getCategory()==cat)//Filtered Stream<Product> according to specified category
				   .sorted(Comparator.comparing(Product::getManufactureDate));//(p1,p2)->p1.getManufactureDate().compareTo(p2.getManufactureDate())  Sorted Stream<Product> according to manufactureDate				   
                                                                  //(p->p.getManufactureDate())
	}
		}
